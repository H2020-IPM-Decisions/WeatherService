...
/**
 * Fetches gridded seasonal weather data and forecasts for in Sweden. The
 * service is provided by the Fältforsk database (https://www.slu.se/faltforsk)
 * at the Swedish University of Agricultural Sciences (https://www.slu.se/en/).
 * It is compatible with the VIPS weather service protocol, and we're
 * adapting it to IPM Decisions in this class.
 * 
 * In the Swedish growing season (March-October) 36 hours of weather forecasts
 * are included.
 * 
...
public class SLULantMetAdapter {
	 ...
	 * outputType = [JSON(default),CSV] JSON is the VIPS standard weather data
	 * format
	 * inputType = [OBSERVED (default),GRID,GRID11]
	 * logIntervalId = 1 (hourly) or 2 (daily)
	 * startDate = YYYY-mm-dd
	 * endDate = YYYY-mm-dd (set this to a couple of days into the future to get
	 * weather forecasts in the Swedish growing season March-October)
	 * elementMeasurementTypeList = comma separated list of weather parameters (VIPS
	 * parameter codes)
	 * nDegrees and eDegrees = defining the square around the centerWGSn/e to
	...
	private Integer[] defaultParameters = {
			...
			// 3101, // Leaf wetness (min/hour) // TODO reinstate this when you know that
			// LantMet can handle it
	 ...
	 */
	public WeatherData getData(Double longitude, Double latitude, Instant timeStart, Instant timeEnd, Integer interval,
			List<Integer> parameters) throws DatatypeConfigurationException, IOException, WeatherDataSourceException {
		...
		// Latitude and longitude needs to be multiplied by 1000 and rounded to nearest
		// Integer
